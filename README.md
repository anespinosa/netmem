...
[![](https://img.shields.io/badge/devel%20version-1.1--0-red.svg)](https://github.com/anespinosa/netmem)
[![Lifecycle:
...
5.  `structural_holes()`: Effective size, efficiency and constraint of
    every node, with overlapping categories

6.  `alter_composition()`: Categories of the alters, which can overlap

7.  `alter_heterogeneity()`: Heterogeneity of the alters, with
    overlapping categories

8.  `alter_homophily()`: E-I index and Yule’s Q of every node, with
    overlapping categories

9.  `brokerage_roles()`: Gould and Fernandez brokerage roles, with
    overlapping categories
...
10. `krackhardt_index()`: Connectedness, hierarchy, efficiency and upper
    boundedness
...
10. `partition_centrality()`: Contribution of each category to the
...
11. `preserved_order()`: Whether a centrality preserves a dominance
    relation
...
1.  `segregation()`: Assortativity, Gupta-Anderson-May, odds ratio,
    Coleman and Freeman

Social influence and diffusion:

1.  `social_influence()`: Assimilation, bounded confidence, repulsion
